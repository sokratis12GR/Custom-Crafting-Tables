...
import static net.thedragonteam.cct.compat.crafttweaker.CTCCTPlugin.toSixBySixShapedObjects;
...
public class SixBySix {
    ...
    public static void addShaped(IItemStack output, IIngredient[][] ingredients) {
        CraftTweakerAPI.apply(new Add(new BaseShapedOreRecipe(6,toStack(output), toSixBySixShapedObjects(ingredients))));
    }
    ...
    private static class Add extends AddUndoableAction {
        ...
        @Override
        public void apply() {
            BaseCraftingManager.getSixBySix().getRecipeList().add(recipe);
        }
    }

    private static class Remove extends RemoveUndoableAction {
        ItemStack remove;
        List<IRecipe> recipeList = BaseCraftingManager.getSixBySix().getRecipeList();

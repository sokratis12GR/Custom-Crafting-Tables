...
public class ContainerSixBySix extends ContainerTableBase {

    private static final int ITEM_BOX = 18;
    private static final int RECIPE_SLOTS = 37;
    private static final int RECIPE_SIZE = 6;
    private static final int RECIPE_SIZE_TOTAL = 36;
    ...
    public InventoryCraftingImproved craftMatrix = new InventoryCraftingImproved(this, 6, 6);
    public IInventory craftResult = new InventoryCraftResult();

    public ContainerSixBySix(InventoryPlayer playerInventory, TileEntitySixBySix tile) {
        super(tile, RECIPE_SLOTS, MAIN_INVENTORY_SLOTS, FULL_INVENTORY_SLOTS);
        this.world = tile.getWorld();
        this.addSlotToContainer(new BaseSlotCrafting(BaseCraftingManager.getSixBySix(), playerInventory.player, this.craftMatrix, this.craftResult, 0, 150, 63));
        ...
        for (int k = 0; k < 3; ++k)
            for (int i1 = 0; i1 < ROW_SLOTS; ++i1)
                this.addSlotToContainer(new Slot(playerInventory, i1 + k * 9 + 9, 8 + i1 * ITEM_BOX, 138 + k * ITEM_BOX));

        IntStream.range(0, ROW_SLOTS).mapToObj(l -> new Slot(playerInventory, l, 8 + l * ITEM_BOX, 196)).forEachOrdered(this::addSlotToContainer);
    ...
    public void onCraftMatrixChanged(IInventory inventoryIn) {
        this.craftResult.setInventorySlotContents(0, BaseCraftingManager.getSixBySix().findMatchingRecipe(this.craftMatrix, this.world));
    }

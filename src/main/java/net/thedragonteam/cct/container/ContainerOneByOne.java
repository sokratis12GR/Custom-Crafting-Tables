...
public class ContainerOneByOne extends ContainerTableBase {

    private static final int ITEM_BOX = 18;
    private static final int RECIPE_SLOTS = 2;
    private static final int RECIPE_SIZE = 1;
    private static final int RECIPE_SIZE_TOTAL = 1;
    ...
    public InventoryCraftingImproved craftMatrix = new InventoryCraftingImproved(this, 1, 1);
    public IInventory craftResult = new InventoryCraftResult();

    public ContainerOneByOne(InventoryPlayer playerInventory, TileEntityOneByOne tile) {
        super(tile, RECIPE_SLOTS, MAIN_INVENTORY_SLOTS, FULL_INVENTORY_SLOTS);
        this.world = tile.getWorld();
        this.addSlotToContainer(new BaseSlotCrafting(BaseCraftingManager.getOneByOne(), playerInventory.player, this.craftMatrix, this.craftResult, 0, 107, 35));

        for (int i = 0; i < RECIPE_SIZE; ++i)
            for (int j = 0; j < RECIPE_SIZE; ++j)
                this.addSlotToContainer(new Slot(this.craftMatrix, j + i * RECIPE_SIZE, 49 + j * ITEM_BOX, 35 + i * ITEM_BOX));
    ...
    public void onCraftMatrixChanged(IInventory inventoryIn) {
        this.craftResult.setInventorySlotContents(0, BaseCraftingManager.getOneByOne().findMatchingRecipe(this.craftMatrix, this.world))
        ;
    }
